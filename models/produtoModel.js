...
class ProdutoModel {
    ...
    get produtoValorUnitario() { return this.#produtoValorUnitario; } set produtoValorUnitario(value) {this.#produtoValorUnitario = value;}
    get produtoImagem() { return this.#produtoImagem; } set produtoImagem(value) {this.#produtoImagem = value;}
    ...
    async buscarProduto(id){
        ...
        if(rows.length > 0){
            ...
            row['cat_id'], row['mar_id'], "", "", row['prd_valorunitario'], row['prd_imagem']);
        }
    ...
    async listarProdutos() {
        ...
        if(rows.length > 0){
            for(let i=0; i<rows.length; i++){
                ...
                row['prd_cod'], row['prd_nome'], row['prd_quantidade'], 
                row['cat_id'], row['mar_id'], row['cat_nome'], row['mar_nome'], row['prd_valorunitario'], row['prd_imagem']));
            }
        }
